...
#.  To make a virtual environment in which all other packages are going to be installed first
    ...
    in the following steps we use ``C:\Users\user\Documents``::
        ...
        C:\Users\user\Documents> virtualenv pymote3_env
        New python executable in pymote3_env\Scripts\python.exe
        Installing setuptools................done.
        Installing pip...................done.
    
    This command has made a new directory ``pymote3_env`` inside ``C:\Users\user\Documents`` with
...
#.  Activate environment::
        ...
        C:\Users\user\Documents> pymote3_env\Scripts\activate
        (pymote3_env) C:\Users\user\Documents>
    
    .. note::

        The ``(pymote3_env)`` prefix to prompt in the last line indicates that newly created environment
...
#.  Set ``PYMOTE_ENV`` environment variable as path to ``pymote3_env`` directory. This way all
    ...
    .. note::
        ...
        In Windows Vista and later use command ``setx PYMOTE_ENV "C:\path\to\pymote3_env"`` to save
...
#.  Launch text editor (use `Notepad++ <http://notepad-plus-plus.org/download>`_ or WordPad, do not
    use plain Notepad) and open ``pymote3_env\Scripts\activate.bat`` file. To display the ``.bat``
...
#.  Open ``pymote3_env\Scripts\deactivate.bat`` in text editor and insert line ``set IPYTHONDIR=`` 
...
.. warning::
    ...
    scripts you must restart the Command prompt and reenter/reactivate ``pymote3_env``. If all goes
...
#.  Based on your processor support of `SSE <http://en.wikipedia.org/wiki/Streaming_SIMD_Extensions>`_ 
    ...
    (nosse|sse2|sse3) using ``easy_install`` command::
        ...
        (pymote3_env) C:\Users\user\Desktop> easy_install numpy-1.7.0-[nosse|sse2|sse3].exe
        (pymote3_env) C:\Users\user\Desktop> easy_install scipy-0.11.0-[nosse|sse2|sse3].exe
...
``PLATLIB`` should be copied to ``pymote3_env/Lib/site-packages/`` directory::
...
.. code-block:: bash
    ...
    (pymote3_env)> easy_install pyreadline


PySide
------
For Pymote3 GUI part of the library PySide Qt bindings for Python should be installed. This is 
achieved `using this solution <http://stackoverflow.com/a/4673823/1247955>`__, that is, running 
following commands:

.. code-block:: bash

    (pymote3_env)> easy_install PySide
    (pymote3_env)> python pymote3_env\Scripts\pyside_postinstall.py -install
    
Pymote3
======
Finally, in order to download and install Pymote3 and all other required packages there are two
...
Before starting, make sure that virtual environment is activated :ref:`windows <windows-venvact>` and run ``ipymote3`` for interactive console or ``pymote3-simgui`` for simualtion GUI. For more details refer to :doc:`starting`.


Additional customization
------------------------

The recommended way to avoid starting command prompt, activating the virtual environment and running ``ipymote3`` in it is to make a shortcut to the ``ipymote3.exe`` file on the desktop, taskbar or start menu.
