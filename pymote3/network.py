import inspect
from pymote3.logger import logger
...
from pymote3.utils.helpers import pymote3_equal_objects
...
class Network(Graph):
    ...
    def nodes_list(self, data=False):
        ...
        nodes = super().nodes(data)
        return sorted(list(iter(nodes)), key=lambda k: k.id)
    ...
    def algorithms(self, algorithms):
        ...
        if not isinstance(algorithms, tuple):
            raise Pymote3NetworkError('algorithm')
    ...
    def communicate(self):
        ...
        while self.outbox:
            ...
            if (message.destination == None and message.nexthop == None):
                ...
                except Pymote3MessageUndeliverable as e:
                    print(e.message)
                ...
                #       in connected part of the network
                    ...
                    raise Pymote3MessageUndeliverable('Can\'t deliver message.',
                                                      message)
    ...
    def broadcast(self, message):
        if message.source in self.nodes():
            ...
            raise Pymote3MessageUndeliverable('Source not in network. \
    ...
    def send(self, destination, message):
        ...
        if destination in self.nodes():
            ...
            raise Pymote3MessageUndeliverable('Destination not in network.',
                                             message)
    ...
    def validate_params(self, params):
        ...
        for param, value in list(params.items()):
            if param=='connected':
                ...
                for node in self:
                    assert(all(map(lambda s1, s2: pymote3_equal_objects(s1, s2),
            ...
            """
                ...
                assert(all(map(lambda a1, a2: pymote3_equal_objects(a1, a2),
                               alg, self.algorithms)))
                #restore alg
...
class Pymote3MessageUndeliverable(Exception):
...
class Pymote3NetworkError(Exception):
